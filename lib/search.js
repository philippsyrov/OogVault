...
const TOPIC_KEYWORDS = {
  ...
  'Science': ['physics', 'chemistry', 'biology', 'atom', 'molecule', 'gravity', 'evolution', 'experiment', 'hypothesis', 'quantum', 'star', 'planet', 'energy', 'cell', 'dna', 'electron', 'photon', 'spectrum', 'cosmos', 'galaxy'],
  ...
  'Cooking': ['recipe', 'cook', 'bake', 'ingredient', 'meal', 'kitchen', 'flavor', 'dish', 'food', 'spice', 'sauce', 'oven', 'grill', 'roast', 'seasoning', 'soup', 'pasta'],
...
export function classifyTopic(text) {
  ...
  for (const [category, keywords] of Object.entries(TOPIC_KEYWORDS)) {
    ...
    for (const kw of keywords) {
      if (kw.includes(' ')) {
        // Multi-word keywords: check substring in full text
...
function fuzzyScore(query, text) {
  ...
  // Hard gate: require at least 2 meaningful keyword matches.
  // A single shared word (e.g. "explain") is not enough to be relevant.
 ...
 * @param {string|null} category - If provided, export only that category; otherwise export all grouped by category.
 * @param {Array|null} nuggetsList - Pre-fetched nuggets array (optional; fetches from DB if null).
 */
export async function exportKnowledgeMarkdown(category = null, nuggetsList = null) {
  const nuggets = nuggetsList || await getAllNuggets();
  if (nuggets.length === 0) return null;

  // Ensure every nugget has a category (fallback for old nuggets)
