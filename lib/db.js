    ...
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            if (!db.objectStoreNames.contains('nuggets')) {
                ...
                const nuggetStore = event.target.transaction.objectStore('nuggets');
...
export async function getAllConversations() {
    ...
    return new Promise((resolve, reject) => {
        ...
        req.onsuccess = () => {
            try {
                const conversations = (req.result || []).sort((a, b) => {
...
export async function getAllNuggets() {
    ...
    return new Promise((resolve, reject) => {
        const req = store.getAll();
        req.onsuccess = () => {
            const nuggets = req.result.sort((a, b) => (b.created_at || '').localeCompare(a.created_at || ''));
            resolve(nuggets);
        };
        req.onerror = () => reject(req.error);
    });
...
export async function searchNuggets(query) {
    ...
    return nuggets.filter((n) => n.question.toLowerCase().includes(lower) ||
