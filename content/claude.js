...
(function () {
    ...
    function strategyRoleSelectors(root) {
        ...
        const dedupedUsers = userElements.filter((item) => !userElements.some((other) => other !== item && other.el.contains(item.el)));
    ...
    function strategyTurnBased(root, userItems) {
            ...
            while (current && current !== root && depth < 20) {
                ...
                const siblings = Array.from(parent.children).filter((c) => {
                    if (c === current)
                        return false;
                    try {
                        return (c.innerText || '').trim().length > MIN_TURN_LENGTH;
                    }
                    catch {
                        return false;
                    }
        ...
        if (!turnContainer)
            return [];
        console.log('[OogVault] Turn-based: turn container tag:', turnContainer.tagName, 'children:', turnContainer.children.length);
        ...
        if (allTurns.length < 2 && turnContainer.parentElement && turnContainer.parentElement !== root) {
            ...
            console.log('[OogVault] Turn-based: trying higher container:', higherContainer.tagName, 'substantial children:', higherTurns.length);
            ...
            if (higherContainer.parentElement && higherContainer.parentElement !== root) {
                ...
                console.log('[OogVault] Turn-based: trying top container:', topContainer.tagName, 'substantial children:', topTurns.length);
        ...
        if (userOnly && messages.length > 0) {
            ...
            for (const { id, el } of allTestIds) {
                if (userTestIds.includes(id.toLowerCase())) {
                    const text = (el.innerText || '').trim();
                    if (text.length > 3)
                        userEls.push({ el, role: 'user', text });
                }
            }
            if (userEls.length > 0) {
