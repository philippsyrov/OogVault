...
(function () {
    ...
    function getChatContainer() {
        return (document.querySelector('.max-w-threadContentWidth') ||
    ...
    function strategyQueryAndProse() {
        const messages = [];
        const queryButtons = document.querySelectorAll('button[data-testid="copy-query-button"], button[aria-label="Copy Query"]');
        console.log(`[OogVault] S1: ${queryButtons.length} query buttons`);
        for (const btn of Array.from(queryButtons)) {
            const userRoot = findUserMessageRoot(btn);
            const textEl = userRoot.querySelector('.whitespace-pre-line.break-words') ||
            ...
            if (turnContainer) {
                const proseBlocks = turnContainer.querySelectorAll('.prose, [class*="prose"][class*="text-pretty"], [data-testid="answer"], [data-testid="assistant-message"], [class*="markdown"]');
    ...
    function findTurnContainer(button) {
        ...
        while (node && node !== document.body && depth < 15) {
            ...
            if (cls.includes('threadcontent') || cls.includes('thread-content'))
                break;
    ...
    function strategyClassSelectors() {
        const messages = [];
        const userEls = document.querySelectorAll('.whitespace-pre-line.break-words, [class*="query-text"], [class*="user-query"]');
        const assistantEls = document.querySelectorAll('.prose.text-pretty, [class*="prose"][class*="text-pretty"], [class*="answer-text"], [class*="response-text"]');
        console.log(`[OogVault] S2: ${userEls.length} user elements, ${assistantEls.length} assistant elements`);
        const dedupedUsers = deduplicateElements(Array.from(userEls));
        const dedupedAssistants = deduplicateElements(Array.from(assistantEls));
    ...
    function cleanText(element) {
        ...
        clone
            .querySelectorAll('button, [role="toolbar"], nav, header, footer, [class*="toolbar"], [class*="action-button"], [class*="citation"], svg, [class*="source-card"], [class*="related"]')
            .forEach((el) => el.remove());
        return (clone.textContent || '').trim();
    }
    function deduplicateElements(els) {
    ...
    function logDomDebugInfo() {
        ...
        document.querySelectorAll('[class]').forEach((el) => {
            ...
            if (keywords.test(c))
                matchingClasses.add(c.substring(0, 100));
        });
