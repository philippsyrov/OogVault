...
async function requestPersistence(): Promise<void> {
  ...
  try {
    ...
    console.warn('[OogVault] Persistent storage request failed:', e);
  }
}
 ...
 */
function isConnectionAlive(db: IDBDatabase): boolean {
  try {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event: IDBVersionChangeEvent) => {
      ...
      if (!db.objectStoreNames.contains('nuggets')) {
        ...
        nuggetStore.createIndex('category', 'category', { unique: false });
      } else {
        // v3: Add category index to existing nuggets store
        const nuggetStore = (event.target as IDBOpenDBRequest).transaction!.objectStore('nuggets');
        if (!nuggetStore.indexNames.contains('category')) {
          nuggetStore.createIndex('category', 'category', { unique: false });
        }
      }
    };
    ...
    request.onerror = () => {
      console.error('[OogVault] DB open error:', request.error);
...
export async function saveConversation(conversation: VaultConversation): Promise<VaultConversation> {
  ...
  const convRecord: VaultConversation = {
    id: conversation.id,
  ...
  await new Promise<void>((resolve, reject) => {
    existingMsgs.onsuccess = (event: Event) => {
...
export async function getConversation(id: string): Promise<VaultConversation | null> {
  const db = await openDB();
  const tx = db.transaction(['conversations', 'messages'], 'readonly');
  const convStore = tx.objectStore('conversations');
  const msgStore = tx.objectStore('messages');

  const conversation = await new Promise<VaultConversation | undefined>((resolve, reject) => {
    const req = convStore.get(id);
    req.onsuccess = () => resolve(req.result as VaultConversation | undefined);
    req.onerror = () => reject(req.error);
  });

  if (!conversation) return null;

  const messages = await new Promise<VaultMessage[]>((resolve, reject) => {
    const msgs: VaultMessage[] = [];
    const req = msgStore.index('conversation_id').openCursor(IDBKeyRange.only(id));
    req.onsuccess = (event: Event) => {
      const cursor = (event.target as IDBRequest<IDBCursorWithValue | null>).result;
      if (cursor) {
        msgs.push(cursor.value as VaultMessage);
        cursor.continue();
      } else {
...
export async function getAllConversations(): Promise<VaultConversation[]> {
  const db = await openDB();
  const tx = db.transaction('conversations', 'readonly');
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = () => {
      try {
        const conversations = ((req.result || []) as VaultConversation[]).sort((a, b) => {
...
export async function deleteConversation(id: string): Promise<boolean> {
  ...
  await new Promise<void>((resolve) => {
    msgCursor.onsuccess = (event: Event) => {
      const cursor = (event.target as IDBRequest<IDBCursorWithValue | null>).result;
      if (cursor) {
        cursor.delete();
        cursor.continue();
  ...
  await new Promise<void>((resolve) => {
    tagCursor.onsuccess = (event: Event) => {
      const cursor = (event.target as IDBRequest<IDBCursorWithValue | null>).result;
      if (cursor) {
        cursor.delete();
        cursor.continue();
  ...
  await new Promise<void>((resolve) => {
    nuggetCursor.onsuccess = (event: Event) => {
      const cursor = (event.target as IDBRequest<IDBCursorWithValue | null>).result;
      if (cursor) {
        cursor.delete();
        cursor.continue();
...
export async function getMessagesForConversation(conversationId: string): Promise<VaultMessage[]> {
  const db = await openDB();
...
export async function getAllMessages(): Promise<VaultMessage[]> {
  ...
  return new Promise((resolve, reject) => {
    const req = store.getAll();
    req.onsuccess = () => resolve(req.result as VaultMessage[]);
    req.onerror = () => reject(req.error);
  });
}
...
export async function addTag(conversationId: string, tag: string): Promise<boolean> {
  const db = await openDB();
...
export async function removeTag(conversationId: string, tag: string): Promise<boolean> {
  const db = await openDB();
  const tx = db.transaction('tags', 'readwrite');
  const store = tx.objectStore('tags');
...
export async function getTagsForConversation(conversationId: string): Promise<string[]> {
  const db = await openDB();
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = (event: Event) => {
      const cursor = (event.target as IDBRequest<IDBCursorWithValue | null>).result;
      if (cursor) {
        tags.push(cursor.value.tag as string);
        cursor.continue();
      } else {
        resolve(tags);
      }
...
export async function saveNuggets(conversationId: string, nuggets: VaultNugget[]): Promise<boolean> {
  const db = await openDB();
...
export async function getAllNuggets(): Promise<VaultNugget[]> {
  const db = await openDB();
  ...
  return new Promise((resolve, reject) => {
    const req = store.getAll();
    req.onsuccess = () => {
      const nuggets = (req.result as VaultNugget[]).sort(
...
export async function searchNuggets(query: string): Promise<VaultNugget[]> {
...
export async function deleteNuggetsForConversation(conversationId: string): Promise<boolean> {
  const db = await openDB();
  const tx = db.transaction('nuggets', 'readwrite');
  const store = tx.objectStore('nuggets');
  const index = store.index('conversation_id');
...
export async function getStats(): Promise<VaultStats> {
  ...
  const nuggetCount = await new Promise<number>((resolve, reject) => {
