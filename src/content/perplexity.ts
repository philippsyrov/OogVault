...
(function (): void {
  ...
  function strategyQueryAndProse(): ChatMessage[] {
  ...
  function findTurnContainer(button: Element): Element | null {
  ...
  function findUserMessageRoot(button: Element): Element {
    let node: Element | null = button;
    let depth = 0;
  ...
  function strategyClassSelectors(): ChatMessage[] {
    const messages: ChatMessage[] = [];
  ...
  function strategyDataAttributes(): ChatMessage[] {
    ...
    const allTestIds: Array<{ tid: string; el: Element }> = [];
  ...
  function deduplicateElements(els: Element[]): Element[] {
  ...
  function initAutocompleteForPlatform(): void {
    ...
    const findInput = (): HTMLElement | null => {
      return (
        ...
        document.querySelector<HTMLElement>('textarea[placeholder*="ask"]') ||
        document.querySelector<HTMLElement>('textarea[placeholder*="follow"]') ||
        document.querySelector<HTMLElement>('textarea[placeholder*="Follow"]') ||
        document.querySelector<HTMLElement>('textarea[class*="textarea"]') ||
        document.querySelector<HTMLElement>('div[contenteditable="true"][class*="editor"]') ||
