...
(function () {
  ...
  let allConversations: VaultConversation[] = [];
  let currentView: 'list' | 'detail' | 'knowledge' = 'list';
  let currentConversationId: string | null = null;
  let activeTab: 'conversations' | 'knowledge' = 'conversations';
  ...
  const viewKnowledge = document.getElementById('view-knowledge') as HTMLElement;
  const convListEl = document.getElementById('conversations-list') as HTMLElement;
  const nuggetsListEl = document.getElementById('nuggets-list') as HTMLElement;
  ...
  const detailTags = document.getElementById('detail-tags') as HTMLElement;
  ...
  const tabConversations = document.getElementById('tab-conversations') as HTMLElement;
  const tabKnowledge = document.getElementById('tab-knowledge') as HTMLElement;
  const btnBack = document.getElementById('btn-back') as HTMLButtonElement;
  const btnSettings = document.getElementById('btn-settings') as HTMLButtonElement;
  const btnCopy = document.getElementById('btn-copy') as HTMLButtonElement;
  const btnExport = document.getElementById('btn-export') as HTMLButtonElement;
  const btnDelete = document.getElementById('btn-delete') as HTMLButtonElement;
  const btnExportKnowledge = document.getElementById('btn-export-knowledge') as HTMLButtonElement;
  ...
  async function loadConversations(): Promise<void> {
    ...
    allConversations = (response?.conversations as VaultConversation[] | undefined) || [];
  ...
  async function updateStats(): Promise<void> {
    ...
    if (response?.stats) {
      const s = response.stats as VaultStats;
  ...
  function onSearch(): void {
    ...
    searchTimer = setTimeout(async () => {
      ...
      if (response?.results) {
        renderSearchResults(response.results as SearchConversationResult[]);
      }
    }, 250);
  }
  ...
  function renderConversationList(conversations: VaultConversation[]): void {
    ...
    convListEl.querySelectorAll('.conv-card').forEach((card) => {
      card.addEventListener('click', () => {
        showDetailView((card as HTMLElement).dataset.id!);
      });
    });
  }

  function renderSearchResults(results: SearchConversationResult[]): void {
    if (results.length === 0) {
      convListEl.innerHTML = `
        <div class="empty-state">
          <div class="empty-state-icon">
  ...
  function renderConvCard(conv: VaultConversation, matchedContent?: string): string {
    const platformMap: Record<string, string> = { claude: 'C', chatgpt: 'G', gemini: 'Ge', perplexity: 'P' };
  ...
  async function showDetailView(conversationId: string): Promise<void> {
    ...
    (document.querySelector('.search-bar') as HTMLElement).style.display = 'none';
    ...
    const conv = response.conversation as VaultConversation;
    const messages: VaultMessage[] = conv.messages || [];
    ...
    renderTags((tagsResp?.tags as string[] | undefined) || []);
  }
  ...
  function renderTags(tags: string[]): void {
    ...
    detailTags.querySelectorAll('.tag-remove').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        renderTags((resp?.tags as string[] | undefined) || []);
      });
    });
  ...
  function switchTab(tab: 'conversations' | 'knowledge'): void {
    activeTab = tab;
    ...
    viewKnowledge.style.display = 'none';
    (document.querySelector('.search-bar') as HTMLElement).style.display = 'block';
  ...
  async function loadNuggets(query?: string): Promise<void> {
    ...
    const nuggets = (response?.nuggets as VaultNugget[] | undefined) || [];
    renderNuggets(nuggets, hasQuery);
  }

  function renderNuggets(nuggets: VaultNugget[], isSearch: boolean = false): void {
    if (nuggets.length === 0) {
    ...
    const grouped: Record<string, VaultNugget[]> = {};
    ...
    nuggetsListEl.querySelectorAll('.category-header').forEach((header) => {
      header.addEventListener('click', (e: Event) => {
        if ((e.target as HTMLElement).closest('.category-export-btn')) return;

        const catId = (header as HTMLElement).dataset.catId;
    ...
    nuggetsListEl.querySelectorAll('.category-export-btn').forEach((btn) => {
      btn.addEventListener('click', async (e: Event) => {
        e.stopPropagation();
        const category = (btn as HTMLElement).dataset.exportCategory!;
  ...
  async function exportKnowledge(): Promise<void> {
    ...
    if (response?.markdown) {
      const blob = new Blob([response.markdown as string], { type: 'text/markdown' });
  ...
  async function exportKnowledgeCategory(category: string): Promise<void> {
  ...
  async function copyConversation(): Promise<void> {
    ...
    const messages: VaultMessage[] = (response.conversation as VaultConversation).messages || [];
    const text = messages
  ...
  function groupByDate(conversations: VaultConversation[]): Record<string, VaultConversation[]> {
    const groups: Record<string, VaultConversation[]> = {};
